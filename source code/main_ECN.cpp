...
#define INIT_REQUEST_LIST                                          \
    {                                                              \
        {"PC7", "PC3", 1000, 10000}, { "PC4", "PC6", 1000, 10000 } \
...
#define ECN_THRESHOLD 200 // 定义ECN阈值，单位为包数目，超过此值则启动ECN机制
                ...
                for (auto it = req_queue_map.begin(); it != req_queue_map.end(); it++)
                {
                    if (it->second.request_list.size() > 0)
                    {
                        if (std::chrono::high_resolution_clock::now() >= it->second.next_send_time && key_pool_map[it->first].encrypt_key_num > 0)
                        {
                            ...
                            // 当密钥量小于ECN阈值时，发送ECN包
                            if (key_pool_map[it->first].encrypt_key_num < ECN_THRESHOLD)
                            {
                                // 如果源节点不是当前节点，发送ECN包
                                if (now_node.ip_name != it->second.next_request_src_name)
                                {
                                    packet ecn_packet(inet_addr(dst_map[it->second.next_request_src_name].c_str()), inet_addr(dst_map[it->second.next_request_dst_name].c_str()), ECN_PACKET, 0);
                                    ecn_packet.set_key_zero();
                                    if (sockfd_map[it->second.next_request_src_name] == -1)
                                    {
                                        sockfd_map[it->second.next_request_src_name] = pre_send(dst_map[it->second.next_request_src_name].c_str(), KEYRELAY_PORT);
                                    }
                                    while (ecn_packet.send_data(sockfd_map[it->second.next_request_src_name]) == -1)
                                    {
                                        std::this_thread::sleep_for(std::chrono::microseconds(int(KEYFILE_SIZE * 8 * 1000 / key_pool_map[it->first].key_rate * 1000)));
                                    }
                            ...
                            // it->second.next_send_time = std::chrono::high_resolution_clock::now() + std::chrono::microseconds(5000);
                        }
                    }
                }
